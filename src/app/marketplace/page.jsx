...
// function adaptTokenToListing(token) {
//   return {
//     id: token.token_id,
//     tokenId: token.token_id,
//     assetContractAddress: token.contract_address,
//     assetContract: token.contract_address,
//     pricePerToken: null, // no price yet
//     _raw: token,
//   };
// }
...
//   //   load();
//   // }, []);
// // Replace the load useEffect with this:
// useEffect(() => {
//   async function load() {
//     try {
//       const { getMarketplaceTokens } = await import('../../lib/db');
//       const rows = await getMarketplaceTokens();
...
function ListingCard({ token, walletAddress, isWishlisted, onWishlistToggle }) {
  const [wishlistLoading, setWishlistLoading] = useState(false);
  ...
  async function toggleWishlist() {
    ...
    try {
      if (isWishlisted) {
        ...
        onWishlistToggle(token.token_id, false);
      } else {
        await supabaseBrowser
          .from('wishlists')
          .insert({ wallet_address: walletAddress, token_id: token.token_id });
        onWishlistToggle(token.token_id, true);
      }
    } catch (err) {
      console.error('Wishlist error:', err);
    } finally {
    ...
    >
      ...
      <div style={{ padding: '18px 20px 22px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        {token.primary_artist && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontFamily: 'var(--font-sans)', fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--cream-25)' }}>Artist</span>
            <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.82rem', color: 'var(--cream-50)' }}>{token.primary_artist}</span>
          </div>
        )}

        {token.release_year && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontFamily: 'var(--font-sans)', fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--cream-25)' }}>Year</span>
            <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.82rem', color: 'var(--cream-50)' }}>{token.release_year}</span>
          </div>
        )}

        {token.vinyl_condition && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontFamily: 'var(--font-sans)', fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--cream-25)' }}>Condition</span>
            <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.82rem', color: 'var(--cream-50)' }}>{token.vinyl_condition}</span>
          </div>
        )}
...
export default function MarketplacePage() {
  ...
  // Load wishlist when wallet connects
  useEffect(() => {
    if (!walletAddress) { setWishlist(new Set()); return; }
    async function loadWishlist() {
      try {
        ...
        setWishlist(new Set((data ?? []).map(r => r.token_id?.toString())));
      } catch (err) {
  ...
  function handleWishlistToggle(tokenId, isNowSaved) {
    setWishlist(prev => {
      const next = new Set(prev);
      isNowSaved ? next.add(tokenId.toString()) : next.delete(tokenId.toString());
      return next;
    });
  }
    ...
    <main style={{ minHeight: '100vh', background: 'var(--surface-00)', color: 'var(--cream)' }}>
      ...
      <section style={{ padding: '60px var(--page-px) 100px' }}>
        {status === 'loading' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', padding: '80px 0', color: 'var(--cream-50)', fontFamily: 'var(--font-sans)', fontSize: '13px', letterSpacing: '0.1em' }}>
            <SpinnerSVG />
            <p>Loading records from the vault…</p>
          </div>
        )}

        {status === 'error' && (
          <div style={{ textAlign: 'center', padding: '80px 0', color: '#e05050', fontFamily: 'var(--font-sans)', fontSize: '13px' }}>
            Failed to load records. Check console for details.
          </div>
        )}

        {status === 'empty' && (
          <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--cream-25)', fontFamily: 'var(--font-sans)', fontSize: '13px', letterSpacing: '0.1em' }}>
            No records in the vault at the moment.
          </div>
        )}
          ...
          <div style={{
            ...
            {tokens.map((token) => (
              <ListingCard
                ...
                token={token}
                walletAddress={walletAddress}
                isWishlisted={wishlist.has(token.token_id?.toString())}
                onWishlistToggle={handleWishlistToggle}
              />
            ))}
          </div>
